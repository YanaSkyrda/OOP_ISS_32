...
public class CheckersActivity extends AppCompatActivity {
    ...
    private void highlightCells(List<Cell> cellsToHighlight) {
        ...
        for(int i = 0; i < cellsToHighlight.size(); i++) {
            int xCord = cellsToHighlight.get(i).getxCord();
            int yCord = cellsToHighlight.get(i).getyCord();
    ...
    private void unhighlightCells(List<Cell> cellsToUnhighlight) {
        ...
        for(int i = 0; i < cellsToUnhighlight.size(); i++) {
            int xCord = cellsToUnhighlight.get(i).getxCord();
            int yCord = cellsToUnhighlight.get(i).getyCord();
    ...
    private List<Cell> findPossibleMovesCells(Checker currChecker, boolean isComputerSide) {
        ...
        for(int i = 0; i < possibleMovesCells.size(); i++){
            //IF THEY ARE NOT NEIGHBOURS => CHECKER CAN BEAT ANOTHER CHECKER => PRIORITY TURN
            if(!Utils.areCellsAreNeighbours(possibleMovesCells.get(i), currCell)){
                realPossibleMovesCells.add(possibleMovesCells.get(i));
            }
        }
    ...
    private Cell findSourceCellForComputer() {
        ...
        for(int i = 0; i < computerCheckers.size(); i++) {
            Checker currentChecker = computerCheckers.get(i);
            ...
            if(currentPossibleMoves.size() > 0) {
                ...
                for(int j = 0; j < currentPossibleMoves.size(); j++) {
                    ...
                    if(!Utils.areCellsAreNeighbours(currentCell, currentPossibleMoves.get(j))) {
                        return outputCell;
                    }
                }
            }
